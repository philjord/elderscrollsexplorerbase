...
public class PhysicsSystem implements NbccProvider, PhysicsSystemInterface {
	...
	public PhysicsSystem(	InstRECOStore instRECOStore, AvatarCollisionInfo avatarCollisionInfo,
							BranchGroup behaviourRoot, MeshSource meshSource) {
		...
		physicsSimThread = new PeriodicThread("Physics Sim Thread", MIN_TIME_BETWEEN_STEPS_MS,
					...
					public void runUpdate() {
						if (DEBUG_UPDATE_STEP_SKIP == 1) {
							...
							if (System.currentTimeMillis() - startOfLastUpdate > 300)
								System.err.println(
										"Physics Sim Thread slow " + (System.currentTimeMillis() - startOfLastUpdate));
						...
						try {
							...
							System.out
									.println("PhysicsSystem.physicsTick() exception " + e + " " + e.getStackTrace()[0]);
							e.printStackTrace();
						}
					}
				});
	...
	public void cellChanged(int cid, J3dCELLGeneral cell) {
		...
		if (cell != null && cid != -1) {
			physicsLocaleDynamics = new PhysicsDynamics(instRECOStore, gravity, behaviourRoot, avatarCollisionInfo,
					meshSource);
			loadJ3dCELL(cell);
	...
	private void load(SparseArray<J3dRECOInst> j3dRECOsById) {
		if (physicsLocaleDynamics == null) {
			new Throwable("physicsLocaleDynamics == null has cellChanged(int cid, J3dCELLGeneral cell) been called?")
					.printStackTrace();
			return;
		}
		//System.out.println("load request for cell " + cell.getName());
		...
		for (int i = 0; i < j3dRECOsById.size(); i++) {
			...
			if (bnm instanceof NBSimpleDynamicModel || bnm instanceof NBNonControlledChar) {
				...
				/*	if(bnm instanceof NBSimpleDynamicModel) {
						...
						//  is this dyn being added with a trans of 0,0,0, if so that's not cool, but no longer an issue
		...
		if (physicsLocaleDynamics != null) {
			...
			for (PhysicsUpdate pu : cpl) {
				if (pu.type == PhysicsUpdate.UPDATE_TYPE.LOAD_FROM_MODEL
					|| pu.type == PhysicsUpdate.UPDATE_TYPE.LOAD_FROM_MODEL_DYN) {
					...
					//		physicsLocaleDynamics.unpause();
							...
							|| pu.type == PhysicsUpdate.UPDATE_TYPE.UNLOAD_FROM_MODEL_DYN) {
					//	boolean prevIsPaused = physicsLocaleDynamics.isPaused();
					//	physicsLocaleDynamics.pause();
					// assumes cell id and stmodel set properly by now
					for (int i = 0; i < pu.collection.size(); i++) {
			...
			if ((System.currentTimeMillis() - startOfEventProcessing) > 100)
				System.out.println(
						"Physics event processing is slow " + (System.currentTimeMillis() - startOfEventProcessing));
